...
int winmain::WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd)
{
	...
	windowHandle = CreateWindowExW(dwExStyle, windowClass, windowName,
		WS_CHILD | WS_CLIPSIBLINGS, 0, 0, 600, 416, solitude::parent, nullptr, hInstance, nullptr);
	...
	auto menuHandle = LoadMenuW(hInstance, L"MENU_1");
	...
	RECT hostRect{}; GetClientRect(solitude::parent, &hostRect);
	MoveWindow(hwnd_frame,0,0,hostRect.right,hostRect.bottom,FALSE);
	SetTimer(hwnd_frame,777,1000,nullptr);
...
LRESULT CALLBACK winmain::message_handler(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	...
	auto hostResult=solitude::message(hWnd,Msg,wParam,lParam,hostHandled);
	if(hostHandled)return hostResult;
		...
		case WM_CREATE:
			{
				...
				auto voiceCount = max(1,min(8,options::get_int(nullptr, "Voices", 8)));
	...
	case WM_KEYDOWN:
		...
		case VK_ESCAPE:
			if (options::Options.FullScreen)
				PostMessageW(GetParent(solitude::parent), WM_APP + 20, VK_F4, 0);
...
{
	if (solitude::paintDC) return solitude::paintDC;
...
void winmain::new_game()
{
	...
	pb::replay_level(0);
	// During the opening light show, the first replay cancels that show. Apply
	// a changed player count immediately instead of waiting for another New Game.
	if (pb::MainTable->PlayerCount != options::Options.Players) pb::replay_level(0);
	SetCursor(prevCursor);
}
