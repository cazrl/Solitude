...
void fullscrn::init(int width, int height, int isFullscreen, HWND winHandle, HMENU menuHandle, int changeDisplay)
{
	...
	if (solitude::parent) { screen_mode=0; MenuEnabled=1; window_size_changed(); return; }
...
{
	if (solitude::parent) { window_size_changed(); force_redraw(); return 0; }
	int result = 0;
