...
void options::ReadOptions()
{
	...
	Options.Players = max(1,min(4,Options.Players));
	...
	Options.AlternativeRender = get_int(nullptr, "Alternative Render", true);
...
static std::wstring wide(const char* text) {
    if(!text)return L"Pinball";
    int n=MultiByteToWideChar(CP_ACP,0,text,-1,nullptr,0);
    std::wstring result(n,L'\0'); MultiByteToWideChar(CP_ACP,0,text,-1,result.data(),n);
    if(n)result.resize(n-1); return result;
}
int options::get_int(LPCSTR section,LPCSTR name,int fallback) {
    return GetPrivateProfileIntW(wide(section).c_str(),wide(name).c_str(),fallback,solitude::settings);
}
void options::set_int(LPCSTR section,LPCSTR name,int value) {
    WritePrivateProfileStringW(wide(section).c_str(),wide(name).c_str(),std::to_wstring(value).c_str(),solitude::settings);
}
void options::get_string(LPCSTR section,LPCSTR name,LPSTR output,LPCSTR fallback,int size) {
    wchar_t buffer[4096]{};
    GetPrivateProfileStringW(wide(section).c_str(),wide(name).c_str(),wide(fallback).c_str(),buffer,4096,solitude::settings);
    if(!WideCharToMultiByte(CP_ACP,0,buffer,-1,output,size,nullptr,nullptr))lstrcpynA(output,fallback,size);
}
void options::set_string(LPCSTR section,LPCSTR name,LPCSTR value) {
    WritePrivateProfileStringW(wide(section).c_str(),wide(name).c_str(),wide(value).c_str(),solitude::settings);
}
void options::get_string(LPCSTR section,LPCWSTR name,LPWSTR output,LPCWSTR fallback,int size) {
    GetPrivateProfileStringW(wide(section).c_str(),name,fallback,output,size,solitude::settings);
}
void options::set_string(LPCSTR section,LPCWSTR name,LPCWSTR value) {
    WritePrivateProfileStringW(wide(section).c_str(),name,value,solitude::settings);
}
