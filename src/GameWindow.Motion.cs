...
public sealed partial class GameWindow
{
    ...
    private sealed record CardFlight(CardPose From,CardPose To,double Start,double Duration,bool Arc,bool Period=false,CardPose? Via=null,IReadOnlyList<(double Time,CardPose Pose)>? Stops=null)
    {
        ...
        public CardPose Sample(double now)
        {
            ...
            if(Arc)r.Y-=MathF.Sin(MathF.PI*t)*Math.Min(9,Math.Abs(From.Rect.X-To.Rect.X)*.025f);
            Card card=To.Card;
    ...
    private Dictionary<int,CardPose> CaptureLayout()
    {
        ...
        {
            int slot=i-(Game.State.Waste.Count-fan);var r=TopCard(1);r.X+=Math.Max(0,slot)*(skin.Vista?19:16);
        ...
        {
            var pile=Game.State.Foundations[f];var r=Kind==GameKind.FreeCell?CellRect(f,true):Kind==GameKind.Spider?new RectangleF(Table.Left+16+f*(skin.Vista?CardWidth*.42f:24),Table.Bottom-CardHeight-12,CardWidth,CardHeight):TopCard(f+3);
    ...
    private void BeginCardMotion(bool deal=false)
    {
        ...
        bool animate=skin.Vista?Preferences.Animate:ClassicSpider && Preferences.SpiderAnimateDeal;
        if(animate)
        {
            int dealt=0,home=0;
            foreach(var target in targets.Values.OrderBy(p=>deal?p.Position.Index*12+p.Position.Pile:p.Layer))
            {
                ...
                {
                    double duration=skin.Vista?Preferences.MotionDuration/1000.0:.08,delay=deal?dealt++*(skin.Vista?.009:.012):0;
                    if(!deal && target.Position.Kind==PileKind.Foundation && !dragged)
                    {delay=Kind==GameKind.FreeCell?home++*.055:Kind==GameKind.Spider?.1:0;}
                    if(!moved)duration*=.8;
                    flights[target.Card.Key]=new(from,target,now+delay,duration,skin.Vista && !dragged && moved,!skin.Vista);
                }
            }
        }
