...
public sealed partial class GameWindow
{
    ...
    {
        if(!skin.Modern || !Preferences.DisplayTips || once && !vistaTipsSeen.Add(title))return;
    ...
    {
        if(!skin.Modern || !Preferences.DisplayTips || vistaTipTitle==null || dialog!=DialogPage.None || showingVictory)return;
