# Windows card-game mechanics audit — Solitude 0.5.0
...
- **Resource evidence:** three preserved XP executables were downloaded into ignored `references/Mechanics/`, renamed `.resource.bin`, hashed, and scanned for strings. They were never executed. Menu/dialog strings corroborate controls and options, not algorithms or binary authenticity.
...
Classic presets expose a face-down card until it is clicked; Vista's default flips it as part of the move. Classic Undo retains one state; Vista retains up to 200. Displaying a menu or modal dialog, minimizing, and losing focus pause Solitude's timer. The precise historical focus/pause policy and Vista scoring variants remain comparison gaps. Safe Collect is a Solitude convenience with conservative eligibility; it is not presented as proof of the original right-click scan order.
...
**Requested behavior change:** double-click moves only the selected, exposed card. It deliberately does not trigger FreeCell's normal automatic-home sweep. Explicit Collect advances one eligible card per animation step. This is a user-requested control policy, not a claim that every original FreeCell build suppressed automatic-home moves after a double-click. A manual click-and-destination move still invokes the documented safe sweep.
...
Beginner/intermediate/advanced statistics now record separately, including streaks. Previous combined statistics remain available in Overall because old saves cannot reconstruct difficulty attribution. Switching difficulty records an unfinished old game against its **old** difficulty. Abandoning a started game counts as a loss; switching Windows/game presets suspends it instead. Native Spider save-file compatibility, original loss-detection prompts and every original keyboard shortcut are not implemented.
...
The main remaining work for a stronger exactness claim is side-by-side original-OS testing: Me Spider resources/rules; Vista scoring, auto-home ordering and animations; original focus behavior, drag outlines and shortcut tables; exact stock-deal counting; native save/open and loss prompts. Existing [window references](WINDOW-REFERENCES.md) and [asset provenance](ASSET-SOURCES.md) record visual evidence separately. Modern Settings/Feel, per-preset suspension, smooth motion and safe Collect are deliberate additions. Automated checks reduce regressions; they cannot guarantee that no unexpected behavior remains.
